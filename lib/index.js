...
var evaluate = function(vars) {
    var i = 0;
    var str = '';
    var l = 0;
    while(i < argv.format.length) {
        var c = argv.format.charAt(i);
        if(c === '\\') {
            i += 2;
            continue;
        }
        if(typeof vars[c] === 'string') {
            str += argv.format.substring(l, i);
            str += vars[c];
            l = i + 1;
        }
        i += 1;
    }
    str += argv.format.substr(l);
    return str;
};
...
var scrape = function() {
    ...
    var extractCurrentFiles = function(cb) {
        ...
        for(var i = 0; i < $players.length; i += 1) {
            ...
            var filename = argv.directory + '/' + evaluate(vars);
