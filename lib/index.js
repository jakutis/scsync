...
util.puts('gap: ' + vt.get(Math.floor((now - argv.gap) / 1000), Math.floor(now / 1000)) + ' (' + argv.gap + ' milliseconds)');
...
var scrape = function() {
    ...
    var syncDashboardRound = function() {
        extractCurrentFiles(function(err, currentFiles) {
            ...
            if(!nextURL || files.length >= argv.maxTracks || (files.length > 0 && (now - files[files.length - 1].date.getTime()) > argv.gap)) {
