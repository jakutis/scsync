        ...
        step(function() {
            ...
            for(var i = 0; i < $players.length; i += 1) {
                ...
                if(!argv.streamed && $('.actions .download', $players[i]).length > 0) {
                    filename = argv.directory + '/' + uri.substr(1).replace(/\//g, '-');
                    tryDownload('http://soundcloud.com' + uri + '/download', filename, group());
                } else {
                    var dbTracks = w.SC.clientDB.getTracks();
                    for(var j in dbTracks) {
                        if(dbTracks[j].uri === uri) {
                            filename = argv.directory + '/' + uri.substr(1).replace(/\//g, '-');
                            tryDownload(dbTracks[j].streamUrl, filename, group());
                        }
                    }
                }
            }
